...
class NMTNORMCritierion(nn.Module):
    ...
    def criterion(self, dec_outs, labels):
        ...
        if self.confidence < 1:
            ...
            print("tmp_:")
            print(tmp_.shape)
            tmp_.scatter_(1, tdata.unsqueeze(1).long(), self.confidence)  # after tdata.unsqueeze(1) , tdata shape is [N,1]
            gtruth = tmp_.detach()
        print(scores.shape)
        print(gtruth.shape)
    ...
    def forward(self, output_x, output_y, target_x, target_y, target_weight):
        ...
        for idx in range(num_joints):
            coord_x_pred = output_x[:,idx].squeeze()
            coord_y_pred = output_y[:,idx].squeeze()
            coord_gt_x = target_x[:,idx].squeeze()
            coord_gt_y = target_y[:,idx].squeeze()
            weight = target_weight[:,idx].squeeze()

            # print(coord_x_pred.shape)
            # print(coord_gt_x.shape)
            loss += self.criterion(coord_x_pred,coord_gt_x).mul(weight).mean()
            loss += self.criterion(coord_y_pred,coord_gt_y).mul(weight).mean()
        return loss / num_joints
